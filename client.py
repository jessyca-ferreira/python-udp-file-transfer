...
contatos = {} # dicionário que armazena as informações sobre os contatos

# funçoes receive e send
def receive():
    while True:
        try:
            data, address = client.rdt_receive()     # tupla (ack, seq, data). Recebe dados utilizando o canal rdt3.0
            message = data[2] #Extrai a mensagem           
            
            if message == 'bye': # usuario desconectado caso a message seja igual a 'bye'
                print('Você foi desconectado')
                host.close()
                break
            elif message[0] == 'list': # imprime lista de contatos
